        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={prevMonth} className="bg-surface-hover rounded-xl px-2.5 py-1.5 text-muted-foreground border-none cursor-pointer font-semibold hover:bg-accent text-[12px]">◀</button>
              <button onClick={goToday} className="bg-surface-hover rounded-xl px-2.5 py-1.5 text-muted-foreground border-none cursor-pointer font-semibold hover:bg-accent text-[12px]">오늘</button>
              <span className="text-sm font-bold min-w-[90px] text-center">{viewYear}년 {viewMon + 1}월</span>
              <button onClick={nextMonth} className="bg-surface-hover rounded-xl px-2.5 py-1.5 text-muted-foreground border-none cursor-pointer font-semibold hover:bg-accent text-[12px]">▶</button>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-6">
            ...
            <div
              ...
              className={`bg-surface-card rounded-card shadow-card p-6 cursor-pointer transition-all ${filter === null ? "ring-2 ring-[#006BFE]" : "hover:shadow-md"}`}
            >
            ...
            {Object.entries(monthPlatformCounts).map(([plat, cnt]) => {
              ...
              return (
                <div key={plat} onClick={() => setFilter(active ? null : plat)}
                  className={`bg-surface-card rounded-card shadow-card p-6 cursor-pointer transition-all ${active ? "ring-2" : "hover:shadow-md"}`}
              ...
              {filteredMonth.map((s, i) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-4 px-5 py-3 cursor-pointer hover:bg-surface transition-colors ${i > 0 ? "border-t border-border/50" : ""}`}
                  >
                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: s.color }} />
                    <span className="font-bold text-[13px] w-[70px] flex-shrink-0" style={{ color: s.color }}>{s.ins}</span>
